package com.emirates.urp.fixture;
...
public class TestCheckstyle {
  ...
  private static Checker createChecker(Class<? extends AbstractCheck> check,
      List<AuditEvent> errors, Consumer<? super DefaultConfiguration> cfg) {
    try {
  ...
  public TestCheckstyle(Class<? extends AbstractCheck> check,
      Consumer<? super DefaultConfiguration> cfg) {
  ...
  private String assertionErrorMessage(int line, int col, String message) {
    List<String> errorsInLine = errors.stream().filter(e -> e.getLine() == line)
        .filter(e -> e.getColumn() == col).map(AuditEvent::getMessage).toList();
  ...
  private List<String> errorsInFile() {
    List<String> errorsInFile = errors.stream()
        .map(auditEvent -> String.format("%s:%s %s", auditEvent.getLine(), auditEvent.getColumn(),
